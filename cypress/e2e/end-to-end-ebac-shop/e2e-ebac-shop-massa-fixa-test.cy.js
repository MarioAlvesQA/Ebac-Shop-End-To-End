...
context('Fluxo End To End', () => {
    
    // Fluxo End to End - Login com sucesso usando Comando customizado e compra 4 produtos e validação de pedido.
    it('Fluxo End To End - Login com sucesso usando Comando customizado e compra 4 produtos.', () => {
        cy.login(dadosLogin.usuario, dadosLogin.senha); // Método para login usando dados fixos
        cy.logout(); // Método para logout
        cy.login(dadosLogin.usuario, dadosLogin.senha); // Método para login novamente
        cy.get('.page-title').should('contain', 'Minha conta') // Verifica se a página de "Minha conta" foi carregada corretamente 
